...
public class CommandLineHandler {
    public void StartVend() {    // Create a Scanner object
        Scanner scanner = new Scanner(System.in);
        System.out.println("Enter Initial Machine Balance");

        // Read user input
        String initialChange = scanner.nextLine();
        boolean correct = initialChange.matches("[-+]?[0-9]*\\.?[0-9]+");

        while (!correct) {
            System.out.println("Please enter a valid initial balance");
            initialChange = scanner.nextLine();
            correct = initialChange.matches("[-+]?[0-9]*\\.?[0-9]+");
        }
        ...
        while (!nextLine.toLowerCase(Locale.ROOT).
